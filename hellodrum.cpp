...
void HelloDrumMUX_40672::scan()
{
  ...
  for (byte pin = muxNum * 16; pin < (muxNum + 1) * 16; pin++)
  {
    ...
    rawValue2[pin] = analogRead(pin_A);
  }
}
...
void HelloDrum::singlePiezoMUX2(byte sens, byte thre, byte scan, byte mask)
{
  padType[padNum] = Snum;
  piezoValue = rawValue2[pin_1];
  singlePiezoSensing(sens, thre, scan, mask);
}
